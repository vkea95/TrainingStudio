...
public class No220_Contains_Duplicate_III {
    ...
    As a followup question, it naturally also requires maintaining a window of heights k. When t == 0, it reduces to
...
Bucketing means we map a range of values to the a bucket. For example, if the bucket heights is 3, we consider
0, 1, 2 all map to the same bucket. However, if t == 3, (0, 3) is a considered duplicates but does not map to the
 ...
 thumb, just make sure the heights of the bucket is reasonable such that elements having the same bucket is immediately
 considered duplicates or duplicates must lie within adjacent buckets. So this actually gives us a range of possible
  bucket heights, i.e. t and t + 1. We just choose it to be t and a bucket mapping to be num / t.
